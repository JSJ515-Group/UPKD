"""WideResNet backbones with a flattened final feature representation.

The architecture follows the implementation by Wei Yang.
"""
...
class WideResNet(nn.Module):
    ...
    def forward(self, x, is_feat=False, preact=False):
        ...
        # Preserve the original CIFAR-10/CIFAR-100 pooling path.
        if out.size(-2) == 8 and out.size(-1) == 8:
            out = self.avgpool(out)

        # Tiny-ImageNet inputs produce a larger final feature map.
        else:
            out = F.adaptive_avg_pool2d(out, output_size=1)

        # Preserve the actual batch dimension when flattening.
